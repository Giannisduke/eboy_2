...
class WC_Bookings_Ajax {
	...
	public function calculate_costs() {
		...
		wp_send_json( array(
			'result' => 'SUCCESS',
			'html'   => '<div class="alert alert-success" role="alert">' . apply_filters( 'woocommerce_bookings_booking_cost_string', __( 'Booking cost: ', 'woocommerce-bookings' ), $product ) . wc_price( $display_price ) . $price_suffix . '</div>',
		) );
	}
