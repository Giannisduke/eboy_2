...
class Iconic_WSSV_Core_Settings {
	...
	public static function account_getting_started() {
		...
		<div class="iconic-notice iconic-notice--getting-started">
			<form action="" method="post" class="iconic-notice__dismiss">
				...
				<button title="<?php echo esc_attr( __( 'Dismiss', 'iconic-wssv' ) ); ?>">
					<span class="dashicons dashicons-dismiss"></span></button>
			</form>
			<h2>Welcome to <?php echo self::$args['title']; ?>!</a></h2>
			<p><?php _e( "Thank you for choosing Iconic. We've put together some useful links to help you get started:", 'iconic-wssv' ); ?></p>

			<?php self::output_getting_started_links(); ?>

			<p><strong><?php _e( 'Is something not quite right?', 'iconic-wssv' ); ?></strong> <?php printf( __( 'Take a look at our
				<a href="%s" target="_blank">troubleshooting documentation</a>', 'iconic-wssv' ), self::get_docs_url( 'troubleshooting' ) ); ?>. <?php _e( 'There is a permanent link to the plugin documentation in the "Support" section below.', 'iconic-wssv' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public static function output_getting_started_links() {
		...
		<h3><?php _e( 'Getting Started', 'iconic-wssv' ); ?></h3>

		<ol>
			<?php foreach ( $links as $link ) { ?>
				<li>
					<a href="<?php echo esc_attr( self::get_docs_url() . $link['href'] ); ?>?utm_source=iconic-wssv&utm_medium=insideplugin" target="_blank"><?php echo $link['title']; ?></a>
				</li>
			<?php } ?>
		</ol>
	<?php }
	...
	public static function setup_dashboard( $settings ) {
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) ) {
			if ( ! empty( self::$args['cross_sells'] ) ) {
				$cross_sells = Iconic_WSSV_Core_Cross_Sells::run( array(
				...
				$settings['sections']['cross-sells'] = array(
					...
					'section_title'       => __( 'Works Well With...', 'iconic-wssv' ),
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support?utm_source=iconic-wssv&utm_medium=insideplugin', __( 'Submit Ticket', 'iconic-wssv' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), __( 'Read Documentation', 'iconic-wssv' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/account?utm_source=iconic-wssv&utm_medium=insideplugin', __( 'Manage Your Account', 'iconic-wssv' ) );
	}
}